        ...
        if include_prices:
            ...
            for col, volume, volume_text in price_ranges:
                ...
                if price_per_g and not pd.isna(price_per_g):
                    ...
                    info += f"💧{volume_text} = {total_price}₽ (цена - {price_per_g}₽ за грамм)\n"
        ...
        def format_prices(product):
            ...
            for col, vol in price_map:
                ...
                if price_per_g and not pd.isna(price_per_g):
                    ...
                    prices.append(f"• {vol} мл — цена - {price_per_g}₽ за мл = {total}₽")
                else:
...
async def ask_chatgpt(question):
    ...
    try:
        ...
        system_content += (
            ...
            "24. Всегда используй юмор и смайлы! Отвечай как веселая пантера, а не как скучный учебник\n"
